...
export default function ProfileScreen() {
  ...
  const ProfileOption = ({ icon, title, onPress, color, showBadge = false }: any) => (
    <TouchableOpacity 
    ...
    >
      ...
      <Text style={[styles.optionText, { color: isDark ? '#fff' : '#000' }]}>{title}</Text>
      ...
      >
        ...
        <View style={styles.profileHeader}>
          <TouchableOpacity 
            ...
            onPress={() => navigateTo('/(authenticated)/(tabs)/profile//edit-profile')}
            activeOpacity={0.9}
          >
            {isLoading ? (
              <View style={styles.loadingAvatar}>
                <ActivityIndicator color={isDark ? '#fff' : theme.colors.primary} />
              </View>
            ) : (
              <Image 
                source={user?.profilePicture ? { uri: user.profilePicture } : getAvatarPlaceholder()} 
                style={styles.avatar} 
              />
            )}
          </TouchableOpacity>
          ...
          <Text style={[styles.profileName, { color: isDark ? '#fff' : '#000' }]}>
            {isLoading ? t('common.loading', 'Loading...') : user?.names || t('profile.guest', 'Guest')}
          </Text>
          
          <Text style={[styles.profileEmail, { color: isDark ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.6)' }]}>
            {isLoading ? '' : user?.email || ''}
          </Text>
          
          <TouchableOpacity 
            style={[styles.editProfileButton, { backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)' }]}
            onPress={() => navigateTo('/(authenticated)/(tabs)/profile//edit-profile')}
          >
            <Text style={[styles.editProfileText, { color: isDark ? '#fff' : '#000' }]}>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.sectionContainer}>
            ...
            <View style={[styles.section, { backgroundColor: isDark ? '#111' : '#fff' }]}>
              <ProfileOption 
                icon="person-outline" 
              ...
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//change-password')}
          ...
          <View style={styles.sectionContainer}>
            ...
            <View style={[styles.section, { backgroundColor: isDark ? '#111' : '#fff' }]}>
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//language')}
              ...
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//theme')}
              ...
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//help')}
              ...
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//faq')}
                color="#FF9800"
              />
              ...
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//terms')}
                color={theme.colors.info}
              />
            </View>
          </View>
          ...
          <View style={styles.sectionContainer}>
            ...
            <View style={[styles.section, { backgroundColor: isDark ? '#111' : '#fff' }]}>
              <ProfileOption 
                ...
                onPress={() => navigateTo('/(authenticated)/(tabs)/profile//block-account')}
          ...
          <View style={styles.versionContainer}>
            <Text style={[styles.versionText, { color: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)' }]}>
